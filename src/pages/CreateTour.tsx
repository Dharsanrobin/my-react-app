  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div className="bg-white rounded-2xl max-w-md w-full p-6">
      ...
      <div className="flex gap-2">
        <button onClick={onClose} disabled={isDeleting} className="flex-1 px-4 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 font-semibold">
          Cancel
        </button>
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          setError(JSON.parse(errorText).message || `Failed to load matches (${response.status})`);
        } catch {
          setError(`Failed to load matches (${response.status})`);
        }
      }
    } catch {
      ...
      if (response.ok) {
        const updated = matches.map(m =>
          (m.matchId === selectedMatch.matchId || m.id === selectedMatch.id)
            ? { ...m, scoreA: parseInt(scoreA), scoreB: parseInt(scoreB), status: 'COMPLETED' }
            : m
        );
        setMatches(updated);
        ...
        <div className="p-6 border-b border-slate-200 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Total: {matches.length} | Completed: {matchesWithScore.length} | Pending: {matchesWithoutScore.length}
            </p>
          </div>
        ...
        <div className="p-6 border-b border-slate-200 bg-slate-50">
          ...
          <div className="flex flex-wrap gap-2">
            {tour.selectedMembers.map((member, i) => (
              <span key={member.id} className="inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-sm font-medium text-blue-700">
                {i + 1}. {member.name}
              </span>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <button onClick={fetchMatches} className="px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700">Retry</button>
            </div>
          ) : (
      ...
      {selectedMatch && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            ...
            <div className="flex gap-2">
              <button onClick={handleCloseModal} disabled={isSubmitting} className="flex-1 px-4 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 font-semibold">
                Cancel
              </button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <div className="flex gap-2">
          <button onClick={onClose} disabled={isGenerating} className="flex-1 px-4 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50">
            Cancel
          </button>
            ...
            <>
              ...
              <div className="space-y-2">
                {filteredMembers.map((member) => {
                  ...
                  return (
                    <div key={member.id} onClick={() => toggleMember(member)} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${isSelected ? 'border-blue-400 bg-blue-50' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
        ...
        <div className="p-6 border-t border-slate-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => onSave(tour.id, selectedMembers)}
...
const CreateTour: React.FC = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const converted: Tour[] = apiTours.map(t => ({
            ...
            selectedMembers: [],
            isGenerated: false,
            minimumPlayers: t.minPlayers,
            maximumPlayers: t.maxPlayers,
          ...
          setTours(current => {
            let local: Tour[] = [];
            try {
              local = JSON.parse(localStorage.getItem('tours') || '[]');
            } catch {}
            ...
            const merged = converted.map(api => {
              ...
              return ex
                ? {
                    ...api,
                    selectedMembers: ex.selectedMembers || [],
                    status: ex.selectedMembers?.length ? 'active' : api.status,
                    isGenerated: ex.isGenerated || false,
                  }
                : {
                    ...api,
                    selectedMembers: [],
  ...
  useEffect(() => {
    if (tours.length > 0) localStorage.setItem('tours', JSON.stringify(tours));
  }, [tours]);
  ...
  const handleCreateTour = async (e: React.FormEvent) => {
    ...
    const payload = {
      name: tourName.trim(),
      defaultBalance: defaultBalance ? parseFloat(defaultBalance) : 0,
      startDate,
      endDate,
      minPlayers: minimumPlayers ? parseInt(minimumPlayers) : 0,
      maxPlayers: maximumPlayers ? parseInt(maximumPlayers) : 0,
    };
    ...
    try {
      ...
      const newTour: Tour = {
        ...
        selectedMembers: [],
        isGenerated: false,
        minimumPlayers: payload.minPlayers,
        maximumPlayers: payload.maxPlayers,
        defaultBalance: payload.defaultBalance,
      };
  ...
  const handleDeleteTour = async (id: string) => {
    ...
    try {
      const response = await fetch(`/project/api/tournaments/${parseInt(id.replace('#', ''))}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      const sDate = currentTour?.startDate || new Date().toISOString().split('T')[0];
      const eDate = currentTour?.endDate || sDate;
      ...
      const response = await fetch(`/project/api/tournaments/${parseInt(id.replace('#', ''))}`, {
        ...
        body: JSON.stringify({
          name: editName.trim(),
          defaultBalance: currentTour?.defaultBalance ?? 0,
          startDate: sDate,
          endDate: eDate,
          minPlayers: currentTour?.minimumPlayers ?? 0,
          maxPlayers: currentTour?.maximumPlayers ?? 0,
        }),
      });
  ...
  const handleSaveMembers = async (tourId: string, selectedMembers: Member[]) => {
    ...
    try {
      ...
      if (!teamIds.length) {
        setApiError('No team IDs found');
        setIsAddingMembers(false);
        return;
      }

      const response = await fetch(`/project/api/tournaments/${parseInt(tourId.replace('#', ''))}/teams`, {
        method: 'POST',
        headers: getAuthHeaders(),
      ...
      const updated = tours.map(t =>
        t.id === tourId
          ? { ...t, selectedMembers, status: selectedMembers.length > 0 ? 'active' : t.status }
          : t
      );
      localStorage.setItem('tours', JSON.stringify(updated));
      setTours(updated);
  ...
  const handleGenerateTournament = async () => {
    ...
    try {
      const response = await fetch(`/project/api/tournaments/${parseInt(tourToGenerate.id.replace('#', ''))}/generate-matches`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      ...
      if (!response.ok) {
        const t = await response.text();
        throw new Error(`HTTP error! ${response.status} - ${t}`);
      }
      ...
      const updated = tours.map(t =>
        t.id === tourToGenerate.id ? { ...t, isGenerated: true, status: 'active' as const } : t
      );
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          <form onSubmit={handleCreateTour} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Default Balance</label>
                <input
                  type="number"
                    ...
                    <>
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
                          <span className="text-xs text-slate-500">📅 {tour.startDate ? formatDate(tour.startDate) : formatDate(tour.createdAt)}</span>
                          {tour.endDate && <><span className="text-xs text-slate-400 hidden sm:inline">•</span><span className="text-xs text-slate-500">⏳ Ends: {formatDate(tour.endDate)}</span></>}
                          <span className="text-xs text-slate-400 hidden sm:inline">•</span>
                          <span className="text-xs text-slate-500">🏆 ID: #{tour.id.slice(-4)}</span>
                          {tour.minimumPlayers !== undefined && <><span className="text-xs text-slate-400 hidden sm:inline">•</span><span className="text-xs text-slate-500">👤 Min: {tour.minimumPlayers}</span></>}
                          {tour.maximumPlayers !== undefined && <><span className="text-xs text-slate-400 hidden sm:inline">•</span><span className="text-xs text-slate-500">👥 Max: {tour.maximumPlayers}</span></>}
                          {tour.defaultBalance !== undefined && <><span className="text-xs text-slate-400 hidden sm:inline">•</span><span className="text-xs text-slate-500">💰 ₹{tour.defaultBalance.toLocaleString()}</span></>}
                        ...
                        {tour.selectedMembers && tour.selectedMembers.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {tour.selectedMembers.slice(0, 3).map(m => (
                              <span key={m.id} className="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700 ring-1 ring-blue-200">
                                {m.name}
                              </span>
                            ))}
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => startEdit(tour)} disabled={isUpdating} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-100 disabled:opacity-50">
                          Edit
                        </button>

                        <button onClick={(e) => openDeleteModal(tour, e)} disabled={deletingId === tour.id} className={`rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-100 ${deletingId === tour.id ? 'opacity-50 cursor-not-allowed' : ''}`}>
                          {deletingId === tour.id ? (
                        ...
                        <button onClick={(e) => openMemberModal(tour, e)} disabled={tour.isGenerated} className={`rounded-lg border px-3 py-2 text-xs font-semibold ${
                          tour.isGenerated
                        ...
                        {tour.isGenerated ? (
                          <button onClick={(e) => openViewTournament(tour, e)} className="rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">
                            View
                          </button>
                        ) : (
                          <button onClick={(e) => openGenerateModal(tour, e)} disabled={!tour.selectedMembers || tour.selectedMembers.length < 2} className={`rounded-lg px-3 py-2 text-xs font-semibold ${
